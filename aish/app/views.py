...
def appliance(request):
	...
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	appliance = Product.objects.filter(category='APP')
	return render(request, 'app/appliances.html', {'appliance':appliance, 'totalitem':totalitem})

def essential(request):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	essentials = Product.objects.filter(category='ESS')
...
def book(request):
	...
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	books = Product.objects.filter(category='BT')
